...
class MainActivity : AppCompatActivity() {
    ...
    // --- Mood-card states --------------------------------------------------------
    // The existing mood-label views own both modes: a static bottom label while
    // unpaired and a continuous 45-degree marquee while paired. No extra layout
    // IDs are required, which keeps ViewBinding stable.
    ...
    private fun showUnpairedMoodImmediately() {
        ...
        mine.layoutParams = mine.layoutParams.apply { width = ViewGroup.LayoutParams.MATCH_PARENT }
    ...
    private fun animateToPairedMood() {
        val container = moodPairContainer
        container.post {
            ...
            val partner = binding.columnPartnerStatus
            val gap = 16.dp()
            val targetWidth = ((total - gap) / 2).coerceAtLeast(1)
            val startMine = mine.width.takeIf { it > 0 } ?: total

            partner.visibility = View.VISIBLE
            binding.cardPartnerStatus.visibility = View.VISIBLE
            partner.layoutParams = partner.layoutParams.apply { width = 0 }
            mine.layoutParams = mine.layoutParams.apply { width = startMine }
            showStaticMoodLabels()
            ...
            ValueAnimator.ofFloat(0f, 1f).apply {
                ...
                addUpdateListener { animator ->
                    ...
                    mine.layoutParams = mine.layoutParams.apply {
                        width = (startMine + (targetWidth - startMine) * t).toInt()
                    }
                    partner.layoutParams = partner.layoutParams.apply {
                        width = (targetWidth * t).toInt()
                    }
                }
    ...
    private fun animateToUnpairedMood() {
        ...
        container.post {
            ...
            val mine = myMoodColumn
            val partner = binding.columnPartnerStatus
            if (total <= 0 || partner.visibility != View.VISIBLE) {
                showUnpairedMoodImmediately()
                return@post
            }

            val startMine = mine.width
            val startPartner = partner.width
            ValueAnimator.ofFloat(0f, 1f).apply {
                duration = 360L
                interpolator = AccelerateDecelerateInterpolator()
                addUpdateListener { animator ->
                    val t = animator.animatedValue as Float
                    mine.layoutParams = mine.layoutParams.apply {
                        width = (startMine + (total - startMine) * t).toInt()
                    }
                    partner.layoutParams = partner.layoutParams.apply {
                        width = (startPartner * (1f - t)).toInt()
                    }
                }
                addListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator) {
