...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateLivePreview(emoji: String, animate: Boolean) {
        ...
        binding.tvLivePreviewStaticLabel.text = label
    ...
    private fun displayFriendMood(friendEmoji: String, friendLabel: String, timestampMillis: Long) {
        ...
        binding.tvFriendStaticLabel.text = label
    ...
    private fun startLabelTwinkle() {
        ...
        binding.tvLivePreviewStaticLabel.visibility = View.GONE
        binding.tvFriendStaticLabel.visibility = View.GONE
    }
    ...
    private fun showPairedMoodImmediately() {
        val parent = binding.moodPairContainer
        val half = parent.width / 2
        val myParams = binding.columnMyStatus.layoutParams
        val partnerParams = binding.columnPartnerStatus.layoutParams

        myParams.width = if (half > 0) half else ViewGroup.LayoutParams.MATCH_PARENT
        partnerParams.width = if (half > 0) half else 0

        binding.columnMyStatus.layoutParams = myParams
        binding.columnMyStatus.setPadding(0, 0, 8.dp(), 0)
        binding.columnPartnerStatus.layoutParams = partnerParams
        binding.columnPartnerStatus.setPadding(8.dp(), 0, 0, 0)
        binding.cardPartnerStatus.visibility = View.VISIBLE
        binding.columnPartnerStatus.visibility = View.VISIBLE
        binding.tvLivePreviewStaticLabel.visibility = View.GONE
        binding.tvFriendStaticLabel.visibility = View.GONE
        binding.tvLivePreviewLabel.visibility = View.VISIBLE
        binding.tvFriendStatus.visibility = View.VISIBLE
    ...
    private fun animateToPairedMood() {
        ...
        parent.post {
            ...
            val startMy = binding.columnMyStatus.width.takeIf { it > 0 } ?: total
            val targetMy = total / 2
            val targetPartner = total - targetMy

            binding.columnPartnerStatus.visibility = View.VISIBLE
            binding.cardPartnerStatus.visibility = View.VISIBLE
            binding.columnPartnerStatus.layoutParams =
                binding.columnPartnerStatus.layoutParams.apply { width = 0 }
            binding.columnMyStatus.layoutParams =
                binding.columnMyStatus.layoutParams.apply { width = startMy }

            stopLabelTwinkle()
            binding.tvLivePreviewLabel.visibility = View.INVISIBLE
            binding.tvFriendStatus.visibility = View.INVISIBLE
            binding.tvLivePreviewStaticLabel.visibility = View.VISIBLE
            binding.tvFriendStaticLabel.visibility = View.VISIBLE
            ...
            val animator = ValueAnimator.ofFloat(0f, 1f).apply {
                ...
                addUpdateListener {
                    ...
                    binding.columnMyStatus.layoutParams =
                        binding.columnMyStatus.layoutParams.apply { width = myWidth }
                    binding.columnPartnerStatus.layoutParams =
                        binding.columnPartnerStatus.layoutParams.apply { width = partnerWidth }
                }
    ...
    private fun animateToUnpairedMood() {
        ...
        parent.post {
            ...
            if (total <= 0) {
                binding.columnMyStatus.layoutParams =
                    binding.columnMyStatus.layoutParams.apply { width = ViewGroup.LayoutParams.MATCH_PARENT }
                binding.columnPartnerStatus.layoutParams =
                    binding.columnPartnerStatus.layoutParams.apply { width = 0 }
                binding.columnPartnerStatus.visibility = View.GONE
                binding.tvLivePreviewLabel.visibility = View.GONE
                binding.tvLivePreviewStaticLabel.visibility = View.VISIBLE
                return@post
            }

            val startMy = binding.columnMyStatus.width.takeIf { it > 0 } ?: total / 2
            val animator = ValueAnimator.ofFloat(0f, 1f).apply {
                duration = 360L
                interpolator = AccelerateDecelerateInterpolator()
                addUpdateListener {
                    val t = it.animatedValue as Float
                    binding.columnMyStatus.layoutParams =
                        binding.columnMyStatus.layoutParams.apply {
                            width = (startMy + (total - startMy) * t).toInt()
                        }
                    binding.columnPartnerStatus.layoutParams =
                        binding.columnPartnerStatus.layoutParams.apply {
                ...
                addListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator) {
                        binding.columnMyStatus.layoutParams =
                            binding.columnMyStatus.layoutParams.apply { width = ViewGroup.LayoutParams.MATCH_PARENT }
                        binding.columnMyStatus.setPadding(0, 0, 0, 0)
                        binding.columnPartnerStatus.layoutParams =
                            binding.columnPartnerStatus.layoutParams.apply { width = 0 }
                        binding.columnPartnerStatus.visibility = View.GONE
                        binding.cardPartnerStatus.visibility = View.GONE
