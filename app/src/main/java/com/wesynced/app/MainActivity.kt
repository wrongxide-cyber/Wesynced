...
class MainActivity : AppCompatActivity() {
    ...
    // These views were added to the main layout for the paired/unpaired
    // transition. Resolve them from the root directly so the code remains
    // compatible with the existing ViewBinding setup across generated binding
    // variants/caches.
    private val moodPairContainer get() = binding.root.findViewById<ViewGroup>(R.id.moodPairContainer)
    private val columnMyStatus get() = binding.root.findViewById<ViewGroup>(R.id.columnMyStatus)
    private val columnPartnerStatus get() = binding.root.findViewById<ViewGroup>(R.id.columnPartnerStatus)
    private val tvLivePreviewStaticLabel get() = binding.root.findViewById<TextView>(R.id.tvLivePreviewStaticLabel)
    private val tvFriendStaticLabel get() = binding.root.findViewById<TextView>(R.id.tvFriendStaticLabel)
    ...
    private fun loadSavedPreferences() {
        ...
        if (!savedId.isNullOrEmpty()) {
            ...
            isPartnerConnected = false
            // Keep the single-card/unpaired presentation until Firebase confirms
            // that the other phone is actually present in this pairing room.
            binding.cardPairing.visibility = View.VISIBLE
            binding.cardConnectionBadge.visibility = View.GONE
    ...
    private fun updateLivePreview(emoji: String, animate: Boolean) {
        val label = moodCatalogue[emoji] ?: getString(R.string.custom_mood_fallback_label)
        binding.tvLivePreviewEmoji.setEmoji(emoji)
        binding.tvLivePreviewLabel.setLabelText(label)
        tvLivePreviewStaticLabel.text = label
    ...
    // --- Mood-card marquee ------------------------------------------------------
    // Unpaired cards show a simple static label. Once paired, both mood cards
    // switch to the continuous 45-degree multi-row marquee.
    private fun startLabelTwinkle() {
        if (!::binding.isInitialized || !isPartnerConnected) return
        applyTwinkleAccentColors()
        binding.tvLivePreviewLabel.startTwinkling()
        binding.tvFriendStatus.startTwinkling()
        binding.tvLivePreviewLabel.visibility = View.VISIBLE
        binding.tvFriendStatus.visibility = View.VISIBLE
        tvLivePreviewStaticLabel.visibility = View.GONE
        tvFriendStaticLabel.visibility = View.GONE
    }
    ...
    private fun showPairedMoodImmediately() {
        val parent = moodPairContainer
        val half = parent.width / 2
        val myParams = columnMyStatus.layoutParams
        val partnerParams = columnPartnerStatus.layoutParams

        myParams.width = if (half > 0) half else ViewGroup.LayoutParams.MATCH_PARENT
        partnerParams.width = if (half > 0) half else 0

        columnMyStatus.layoutParams = myParams
        columnMyStatus.setPadding(0, 0, 8.dp(), 0)
        columnPartnerStatus.layoutParams = partnerParams
        columnPartnerStatus.setPadding(8.dp(), 0, 0, 0)
        binding.cardPartnerStatus.visibility = View.VISIBLE
        columnPartnerStatus.visibility = View.VISIBLE
        tvLivePreviewStaticLabel.visibility = View.GONE
        tvFriendStaticLabel.visibility = View.GONE
        binding.tvLivePreviewLabel.visibility = View.VISIBLE
        binding.tvFriendStatus.visibility = View.VISIBLE
        applyTwinkleAccentColors()
        if (isAppInForeground) startLabelTwinkle()
    }
    ...
    private fun animateToPairedMood() {
        ...
        parent.post {
            ...
            if (total <= 0) {
                showPairedMoodImmediately()
                return@post
            }

            val startMy = columnMyStatus.width.takeIf { it > 0 } ?: total
            val targetMy = total / 2
            val targetPartner = total - targetMy

            columnPartnerStatus.visibility = View.VISIBLE
            binding.cardPartnerStatus.visibility = View.VISIBLE
            columnPartnerStatus.layoutParams =
                columnPartnerStatus.layoutParams.apply { width = 0 }
            columnMyStatus.layoutParams =
                columnMyStatus.layoutParams.apply { width = startMy }

            stopLabelTwinkle()
            binding.tvLivePreviewLabel.visibility = View.INVISIBLE
            binding.tvFriendStatus.visibility = View.INVISIBLE
            tvLivePreviewStaticLabel.visibility = View.VISIBLE
            tvFriendStaticLabel.visibility = View.VISIBLE
            ...
            val animator = ValueAnimator.ofFloat(0f, 1f).apply {
                ...
                addUpdateListener {
                    ...
                    val myWidth = (startMy + (targetMy - startMy) * t).toInt()
                    val partnerWidth = (targetPartner * t).toInt()
                    columnMyStatus.layoutParams =
                        columnMyStatus.layoutParams.apply { width = myWidth }
                    columnPartnerStatus.layoutParams =
                        columnPartnerStatus.layoutParams.apply { width = partnerWidth }
                }
    ...
    private fun animateToUnpairedMood() {
        ...
        parent.post {
            ...
            if (total <= 0) {
                columnMyStatus.layoutParams =
                    columnMyStatus.layoutParams.apply { width = ViewGroup.LayoutParams.MATCH_PARENT }
                columnPartnerStatus.layoutParams =
                    columnPartnerStatus.layoutParams.apply { width = 0 }
                columnPartnerStatus.visibility = View.GONE
                binding.tvLivePreviewLabel.visibility = View.GONE
                tvLivePreviewStaticLabel.visibility = View.VISIBLE
                return@post
            }

            val startMy = columnMyStatus.width.takeIf { it > 0 } ?: total / 2
            val animator = ValueAnimator.ofFloat(0f, 1f).apply {
                duration = 360L
                interpolator = AccelerateDecelerateInterpolator()
                addUpdateListener {
                    val t = it.animatedValue as Float
                    columnMyStatus.layoutParams =
                        columnMyStatus.layoutParams.apply {
                            width = (startMy + (total - startMy) * t).toInt()
                        }
                    columnPartnerStatus.layoutParams =
                        columnPartnerStatus.layoutParams.apply {
                            width = (startMy * (1f - t)).toInt()
                        }
                }
                addListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator) {
                        columnMyStatus.layoutParams =
                            columnMyStatus.layoutParams.apply { width = ViewGroup.LayoutParams.MATCH_PARENT }
                        columnMyStatus.setPadding(0, 0, 0, 0)
                        columnPartnerStatus.layoutParams =
                            columnPartnerStatus.layoutParams.apply { width = 0 }
                        columnPartnerStatus.visibility = View.GONE
                        binding.cardPartnerStatus.visibility = View.GONE
                        binding.tvLivePreviewLabel.visibility = View.GONE
                        binding.tvFriendStatus.visibility = View.GONE
                        tvLivePreviewStaticLabel.visibility = View.VISIBLE
                        tvFriendStaticLabel.visibility = View.GONE
                    }
                })
